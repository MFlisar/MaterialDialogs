...
internal class DateTimeEventManager<T : DateTimeData>(
    ...
    override fun onButton(
        ...
        val event = when (setup.value) {
            is DateTimeData.DateTime -> DialogDateTime.EventDateTime.Result(setup.id, setup.extra, viewManager.getCurrentValue() as DateTimeData.DateTime)
            is DateTimeData.Date -> DialogDateTime.EventDate.Result(setup.id, setup.extra, viewManager.getCurrentValue() as DateTimeData.Date)
            is DateTimeData.Time -> DialogDateTime.EventTime.Result(setup.id, setup.extra, viewManager.getCurrentValue() as DateTimeData.Time)
            else -> throw RuntimeException()
        }
        event.send(presenter)
        return true
